class UpdateClaLiveClassesColumns < ActiveRecord::Migration[7.1]
  def up
    ...
    # Add new cla_user_id column as a string with null: false.
    # Using a temporary default value ('TEMP') to avoid not-null violations on existing rows.
    add_column :cla_live_classes, :cla_user_id, :string, null: false, default: 'TEMP'
    add_index :cla_live_classes, :cla_user_id

    # Add new cla_cohort_id column as a string with null: false, with a temporary default.
    add_column :cla_live_classes, :cla_cohort_id, :string, null: false, default: 'TEMP'
    add_index :cla_live_classes, :cla_cohort_id

    # Optionally remove the default so that future records don't get 'TEMP' automatically.
    change_column_default :cla_live_classes, :cla_user_id, nil
    change_column_default :cla_live_classes, :cla_cohort_id, nil

    # Remove the foreign key on cla_course_id before changing its type.
    remove_foreign_key :cla_live_classes, column: :cla_course_id

    # Change cla_course_id column type from uuid to string.
    change_column :cla_live_classes, :cla_course_id, :string
  end

  def down
    # Revert cla_course_id column back to uuid and re-add its foreign key.
