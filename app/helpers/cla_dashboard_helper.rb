module ClaDashboardHelper
  ...
  def self.course_completion_rate(cohort_id)
    total_courses = ClaCourse.where(cla_cohort_id: cohort_id).count
    completed_courses = completed_courses_count(cohort_id)
  ...
  def self.completed_courses_count(cohort_id)
    ...
    # 3. Find courses where at least one submission exists
    completed_course_ids = ClaSubmission.where(cla_assignment_id: assignment_course_map.keys)
