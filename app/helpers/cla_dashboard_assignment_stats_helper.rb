module ClaDashboardAssignmentStatsHelper
  def self.user_submission_percentage(user_id, course_id)
    ...
    course_assignment_ids = ClaAssignment.where(cla_course_id: course_id).pluck(:id)

    # 3. Get the total number of assignments in the cohort
    total_assignments = course_assignment_ids.count

    # 4. Get the number of submissions made by the user
    total_submissions = ClaSubmission.where(cla_student_id: user_id, cla_course_id: course_id).sum(:score)

    # 5. Calculate submission percentage
    submission_percentage = total_assignments.zero? ? 0 : (total_submissions.to_f / (total_assignments * 100) * 100).round(2)
